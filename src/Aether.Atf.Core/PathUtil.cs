...
// Modified 2026 by Resolvora LLC / Aether Engine contributors: replaced shlwapi/Kernel32
// P/Invoke with portable System.IO.Path APIs so the library builds on net10.0 / Linux.
...
namespace Sce.Atf
{
    ...
    public static class PathUtil
    {
        ...
        /// <remarks>This method correctly handles "subst" drive prefixes on Windows in the original ATF.
        /// The portable port uses Path.GetFullPath and does not resolve subst devices.</remarks>
        ...
        /// <remarks>Original ATF resolved Windows subst drives via Kernel32.QueryDosDeviceW.
        /// This port uses Path.GetFullPath only.</remarks>
        ...
        public static string GetCompactedPath(string path, int length)
        {
            ...
            result.Append(path, 0, startLen);
            result.Append("...");
            result.Append(path, path.Length - endLen, endLen);
