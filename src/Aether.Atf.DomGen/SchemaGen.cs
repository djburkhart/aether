...
// - SchemaGenOptions overload so the library is not CLI-shaped
// - StringEnumRule.Values instead of private-field reflection
// - generated text always uses \n so fixtures are stable across OS
...
{
    /// <summary>
    /// Options for SchemaGen.Generate. Parsed from the original DomGen argv (flags after the
    /// first four positional arguments) or set directly by a host.</summary>
    public sealed class SchemaGenOptions
    {
        /// <summary>Generate DomNodeAdapter partial classes with properties (-a / -adapters).</summary>
        public bool GenerateAdapters { get; set; }

        /// <summary>Include types only when sce.domgen include="true" (-annotatedOnly).</summary>
        public bool AnnotatedOnly { get; set; }

        /// <summary>Emit enums for AttributeTypes with string-value restrictions (-enums).</summary>
        public bool GenerateEnums { get; set; }

        /// <summary>Arguments written into the generated "Command Line:" header. Null uses none.</summary>
        public string[] CommandLineArgs { get; set; }

        /// <summary>Parse original DomGen argv: flags begin at index 4.</summary>
        public static SchemaGenOptions FromArgs(string[] args)
        {
            var options = new SchemaGenOptions { CommandLineArgs = args };
            if (args == null)
                return options;
            ...
            for (int i = 4; i < args.Length; i++)
            {
                ...
                if (arg == "-a" || arg == "-adapters")
                    options.GenerateAdapters = true;
                else if (arg == "-annotatedOnly")
                    options.AnnotatedOnly = true;
                else if (arg == "-enums")
    ...
    public static class SchemaGen
    {
        ...
        {
            return Generate(typeLoader, schemaNamespace, codeNamespace, className, SchemaGenOptions.FromArgs(args));
        }
        ...
        /// <param name="typeLoader">Type loader with loaded type and annotation information</param>
        /// <param name="schemaNamespace">Target namespace of the schema</param>
        /// <param name="codeNamespace">Namespace of the class to be generated</param>
        /// <param name="className">Name of the class to be generated</param>
        /// <param name="options">Generation options</param>
        /// <returns>CSharp Schema class as string</returns>
        public static string Generate(SchemaLoader typeLoader, string schemaNamespace, string codeNamespace, string className, SchemaGenOptions options)
        {
            if (options == null)
                options = new SchemaGenOptions();

            bool generateAdapters = options.GenerateAdapters;
            bool annotatedOnly = options.AnnotatedOnly;
            bool generateEnums = options.GenerateEnums;
            string[] args = options.CommandLineArgs ?? Array.Empty<string>();
        ...
        {
            // StringEnumRule marks restricted string AttributeTypes. Values is public on
            // Aether.Atf.Core (upstream DomGen reflected the private m_values field).
            ...
            foreach (AttributeType attributeType in typeLoader.GetAttributeTypes())
            {
                ...
                if (rule != null && enumAttributeTypes.Add(attributeType))
                {
                    string[] values = rule.Values.ToArray();
                    if (values.Length > 0)
                    {
