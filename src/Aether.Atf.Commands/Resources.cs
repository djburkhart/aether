...
// Modified 2026 by Resolvora LLC / Aether Engine contributors: static constructor assigns
// ImageResourceAttribute names only. It no longer reflects for WinForms/WPF ResourceUtil
// (GDI+ image loading is a later tools-host concern).
...
namespace Sce.Atf
{
    ...
    public static class Resources
    {
        ...
        static Resources()
        {
            // Portable port: populate image *names* from ImageResourceAttribute so CommandInfo
            // can register standard commands. Bitmap/icon loading stays in a future tools host.
            foreach (FieldInfo field in typeof(Resources).GetFields(BindingFlags.Public | BindingFlags.Static))
            {
                if (field.FieldType != typeof(string))
                    continue;
                var image = field.GetCustomAttribute<ImageResourceAttribute>();
                if (image != null)
                {
                    field.SetValue(null, image.ImageName1);
                    continue;
                }
                var cursor = field.GetCustomAttribute<CursorResourceAttribute>();
                if (cursor != null)
                    field.SetValue(null, cursor.CursorName);
            }
        }
    }
}
